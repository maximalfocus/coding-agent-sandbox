...
#
#   PASS         the pin is where the inventory says it lives, and every check resting on it is one
#                a repository check can re-run;
#   DRIFTED      a pin baked into the Dockerfile has no inventory row, or a row's recorded pin is no
...
# has no row at all. One parser, so the tool and the check can never disagree.
#
# Exit status: 0 when every baked pin is mapped and every recorded pin is intact,
#              1 when at least one pin has drifted or is unmapped,
#              2 when the inventory itself is missing or malformed (fail closed),
#              3 for --arg only, when the named ARG has no inventory row.
set -uo pipefail
