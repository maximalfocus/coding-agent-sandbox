...
check 'Dockerfile verifies installer before extraction' bash -c "grep -n 'sha256sum -c' '$DOCKERFILE' | grep -q . && grep -n 'unzip' '$DOCKERFILE' | grep -q . && [ \"\$(grep -n 'sha256sum -c' '$DOCKERFILE' | head -1 | cut -d: -f1)\" -lt \"\$(grep -n 'unzip' '$DOCKERFILE' | head -1 | cut -d: -f1)\" ]"
...
check 'default proxy consumes AWS_SSO_REGIONS through helper' contains "$ROOT/entrypoint.sh" 'AWS_SSO_REGIONS'
check 'sidecar proxy consumes AWS_SSO_REGIONS through helper' contains "$ROOT/mitm/sidecar-entrypoint.sh" 'AWS_SSO_REGIONS'
